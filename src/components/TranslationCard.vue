<template>
  <div class="card-wrapper relative mx-auto">
    ...
    <div :class="['translation-card', showFullCard ? 'expanded' : 'collapsed']">
      ...
      >
        <div
          ...
          :class="{ show: showFullCard }"
        >
          <!-- Meaning Section -->
          <transition name="fade-page" mode="out-in">
            <div :key="activePage">
              <h2 class="text-lg font-medium text-gray-400 mb-2">
                <span class="text-black">{{ currentPage.meaning }}</span>
                meaning
              </h2>
              <ul class="text-gray-400 space-y-2 mb-6">
                ...
                >
                  {{ index + 1 }}.
                  <span v-if="index === 1 && !showMore"
                    >{{ desc.substring(0, desc.length / 2) }}...</span
                  >
      ...
      >
        ...
        <button @click="toggleMore" class="text-blue-500 inline ml-2">
          Less
        </button>
      </div>
    ...
    <div v-if="showFullCard" class="icon-container">
      <div
        class="container flex flex-col items-center relative overflow-hidden h-full gap-1 justify-between"
      >
        <!-- Link Icon -->
        ...
        <!-- Languages Icon -->
        <div
          class="translationLanguageSelector bg-[#F3F3F3] mt-0 rounded-lg flex flex-col justify-center items-center rounded overflow-hidden"
        >
          <img
            src="/languages.jpg"
            alt="Languages icon"
          ...
          <span class="language-option text-sm text-gray-950 mt-2 hidden"
            >ES</span
          >
          <span class="language-option text-sm text-gray-950 mt-2 hidden"
            >IT</span
